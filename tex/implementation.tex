...
As discussed in chapter \ref{sec:dijkstra}, the shortest distance can readily be calculated 
using Dijkstra's algorithm \cite{wiki_dijkstra}, which is shown in Fig. \ref{fig:linesegs_path}.
...
Ref. \cite[p. 285]{Erickson2019}. In the present implementation, this search for the next neighbour
...
dijk_mod(const t_graph& graph, const std::string& startvert)
{
	...
	// vector of distances and predecessors
	...
	// heap with vertex indices, sorted by distances to start vertex
	...
	// are there still elements in the heap?
	while(heap.size())
	{
		// get the element on top of the heap
		std::size_t vertidx = *heap.begin();
		std::pop_heap(heap.begin(), heap.end(), vert_cmp);
		heap.pop_back();

		// get the element's neighbours
		auto neighbours = graph.GetNeighbours(vertidx);
...
concept is_graph = requires(t_graph& graph, std::size_t vertidx)
{
	...
	{ graph.GetNumVertices() } -> convertible_to<std::size_t>;

	// function to get vertex identifier from index
	{ graph.GetVertexIdent(vertidx) } -> convertible_to<std::string>;
