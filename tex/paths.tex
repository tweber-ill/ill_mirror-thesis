...
As for a TAS several coordinate systems are available (refer to chapter \ref{ch:xtal}), 
we have several equivalent possibilities to describe its movement. 
...
coordinates that result from the obstacles.
...
On the other hand, using the instrument angles seems to be even more complicated 
at first glance. The instrument comprises six angles, namely the crystal and 
...
the instrument on that path. The two tasks comprise these following steps:
\begin{enumerate}
	...
	\begin{enumerate}
		\item Convert the given crystal coordinates into instrument coordinates.
			The user of a triple-axis spectrometer usually enters the coordinates 
