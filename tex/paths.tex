...
for path-finding without full information is given in Ref \cite[Ch. 7, pp. 313-366]{FUH_geo2020},
...
obstacle's faces as long as the accumulated angle is not at its initial value,
e.g. 0 degrees, again \cite[Ch. 7.2, pp. 316-322]{FUH_geo2020}.

If -- aside from the tactile sensor and the angular sense -- the robot also knows
its current position relative to the target point, it can execute a bug algorithm
to reach its target, which is based on the movement of insects.
...
(CAB) \cite{Icking1995}, which solves the specific problem of finding the visibility kernel
of a robot inside a simple, star-shaped polygon, where the kernel is defined as the
