...
and the underlying metric to the usual Euclidian one \cite[p. 736]{Arens2015},
\begin{equation}
...
\begin{figure}[h]
	...
	\caption[Brillouin zones.]{
		First Brillouin zone for
			(a) a simple cubic \cite[p. 25]{Gross2012},
			(b) a body-centred cubic \cite[p. 26]{Gross2012}, and
			(c) a face-centred cubic \cite[pp. 25-26]{Gross2012} crystal lattice.
...
a point and a line and its parabolic shape is proven in Ref. \cite[Ch. 5, pp. 260-261]{FUH_geo2020}.
...
\begin{itemize}
	...
	\item All requirements were met by \textit{Boost.Polygon} \cite{web_boost_polygon}, though, and we will use 
		...
		The library is internally based on integer coordinates for optimal performance \cite{web_boost_polygon}.
...
\begin{figure}[htb]
	...
	\caption[Voronoi calculation performance.]{
		...
		and the \textit{CGAL / 2D Segment Delaunay Graphs} (labelled ``CGAL'') \cite{web_2dsegdel} libraries.
...
\begin{table}[h]
	...
	\caption[Adjacency matrix and list.]{
		...
		These tables follow the design of equivalent tables in Ref. \cite[p. 5, Fig. 4.3]{FUH_algo_graphs_2021}.}
	\label{tab:example_graph_structure}
\end{table}
% \cite[p. 4, Fig. 4.3]{FUH_algo_graphs_2021}
...
In total, Dijkstra's algorithm is at its core a breadth-first search of a graph \cite{wiki_dijkstra},
where the next neighbour vertex to visit is chosen by the lowest weight factor of the connecting edges.
